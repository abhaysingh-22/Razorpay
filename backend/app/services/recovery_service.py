...
def run_recovery_for_transaction(transaction: dict) -> dict:
    ...
    if action == "retry_scheduled":
        if reason == "bank_timeout":
            ...
            if attempt_num == 1:
                ...
                if random.random() < 0.50:
                    ...
                    result["reasoning"] = (
                        f"Exceeded maximum retries ({attempt_num}) for insufficient funds."
                    )
        ...
        if attempt_num == 1:
            ...
            if random.random() < 0.55:
                ...
                result["reasoning"] = (
                    "Customer updated payment method following notifications. Retry processed successfully."
                )
                new_status = "recovered"
            else:
                if attempt_num >= 3:
                    ...
                    result["action"] = "stopped"
                    result["reasoning"] = (
                        "Customer did not update expired card after multiple reminders."
                    )
                    new_status = "exhausted"
                else:
                    result["outcome"] = "pending"
        ...
        action == "escalated"
        or reason == "fraud_flag"
        or transaction["amount"] >= 10000
    ):
        ...
        if reason == "fraud_flag":
            ...
            result["reasoning"] = (
                "Potential fraud pattern detected. Paused for Human-in-the-Loop Risk Officer verification."
            )
        else:
            result["action"] = "high_ticket_human_review"
            result["reasoning"] = (
                f"High-ticket transaction (₹{transaction['amount']:,.2f}) paused for risk safety review before retry."
            )
    ...
    supabase.table("transactions").update({"status": new_status}).eq(
        "id", transaction["id"]
    ).execute()
...
def resolve_human_review(
    transaction_id: str, decision: str, notes: str | None = None
) -> dict:
    ...
    if decision == "approve":
        ...
        attempt_record = {
            ...
            "reasoning": notes
            or "Risk Officer reviewed risk telemetry and approved recovery retry.",
            "outcome": "success",
        ...
        attempt_record = {
            ...
            "reasoning": notes
            or "Risk Officer confirmed fraud risk and permanently blocked auto-recovery.",
            "outcome": "failed",
            "amount_recovered": 0,
        }
...
def run_batch_recovery() -> dict:
    ...
    res_failed = (
        supabase.table("transactions").select("*").eq("status", "failed").execute().data
        or []
    )
    res_retry = (
