...
def generate_transactions(count: int = 50) -> List[Dict[str, Any]]:
    ...
    for _ in range(count):
        ...
        amount = (
            round(random.uniform(10500, 24000), 2)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic payment failure dataset."
    )
    parser.add_argument(
        "--count",
        "-n",
    ...
    parser.add_argument(
        ...
        "-i",
        action="store_true",
        help="Directly insert generated records into Supabase",
    )
    parser.add_argument(
        "--json", "-j", action="store_true", help="Print output as formatted JSON"
    )
    ...
    if args.insert:
        ...
        print(
            f"✅ Inserted {len(res.data or [])} synthetic transactions into Supabase."
        )
    elif args.json:
        print(json.dumps(data, indent=2))
    else:
        print(
            f"⚡ Generated {len(data)} synthetic transactions (Sample 1st item: {data[0] if data else 'None'})"
        )
