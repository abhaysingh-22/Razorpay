...
def wipe_database() -> None:
    ...
    try:
        supabase.table("recovery_attempts").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
        supabase.table("transactions").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
        supabase.table("batch_summaries").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
...
def generate_mock_transactions(count: int = 30) -> list[dict]:
    ...
    for _ in range(count):
        ...
        amount = round(random.uniform(11000, 25000), 2) if (reason == "fraud_flag" or random.random() < 0.1) else round(random.uniform(299, 4999), 2)
...
def main():
    parser = argparse.ArgumentParser(description="Seed Supabase database with synthetic failed transactions.")
    parser.add_argument("--count", "-c", type=int, default=30, help="Number of transactions to generate (default: 30)")
    parser.add_argument("--wipe", "-w", action="store_true", help="Wipe existing data before seeding")
